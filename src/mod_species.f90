...
!!
!! This module implements non-reacting finite-volume transport for \(N\)
...
!!    after every timestep.  Chemical reactions and source terms are currently
!!    disabled in the supported path.
!! 4. **MPI Synchronization**: Efficiently gathers owned-cell updates into 
!!    the globally replicated mesh field.
module mod_species
   use mod_kinds, only : rk, zero, one, tiny_safe, fatal_error, name_len, lowercase
...
contains
   ...
   !!   conservative \(\rho Y_k\), then divides by the active density.
   !! - **Diffusive fluxes**: constant-density mode uses \(D_k \nabla Y_k\);
   !!   variable-density mode uses \(\rho_f D_k \nabla Y_k\).
   !! - **Correction velocity**: subtracts \(Y_k \sum J_{diff,k}\) at each face
   ...
   subroutine advance_species_transport(mesh, flow, bc, params, fields, species, transport)
      ...
      integer :: k
      real(rk) :: sum_Y
      logical :: is_dirichlet, use_variable_density_species
      ...
      do c = flow%first_cell, flow%last_cell
         ...
         do k = 1, species%nspecies
            if (use_variable_density_species) then
               species%Y(k,c) = (rho_old_cell * species%Y_old(k,c) + &
                                 params%dt * dY(k) / mesh%cells(c)%volume) / rho_cell
            else
