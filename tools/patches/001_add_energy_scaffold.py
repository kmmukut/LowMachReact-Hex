...
inversion, radiation physics, or output changes. It only adds:
  ...
  - optional build-list insertion for common CMake/Makefile layouts
...
def patch_build_lists(repo: Path, backup_root: Path, apply: bool) -> list[PatchResult]:
    ...
    for path in candidates:
        ...
        if "mod_energy.f90" in text:
            results.append(PatchResult(path, False, "build list already mentions mod_energy.f90"))
            continue
        ...
        for line in lines:
            out.append(line)
            if not inserted and "mod_fields.f90" in line:
                out.append(line.replace("mod_fields.f90", "mod_energy.f90"))
                inserted = True

        if inserted:
            new_text = "".join(out)
            results.append(write_if_changed(path, new_text, repo, backup_root, apply))
        else:
            results.append(PatchResult(path, False, "no mod_fields.f90 anchor found; build list not changed"))

    if not results:
        results.append(PatchResult(repo, False, "no common CMakeLists.txt/Makefile found; build list not changed"))
