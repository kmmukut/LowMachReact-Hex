# Enthalpy-Based Energy Equation Roadmap

## Current status

This file is a historical roadmap plus current-state notes. For authoritative
current equations and configuration rules, prefer `doc_src/numerics.md`,
`doc_src/energy_thermo_conventions.md`, `doc_src/cantera_interface.md`, and
`doc_src/input_configuration_guide.md`.

Several items that were originally future work are now implemented:
...
- optional species-diffusion enthalpy correction: -div(sum_k h_k J_k)
- guarded variable-density low-Mach projection with Cantera density
- conservative variable-density rho*Y and rho*h transport
...
4. **Projection has two density modes**
   - The stable baseline uses constant `rho` and targets `div(u) = 0`.
   - The guarded variable-density path syncs `transport%rho <- energy%rho_thermo`
     and targets `div(u) = S_projection`.
   - Variable-density validation should use `divu_minus_S_projection_*`, not raw
     `div(u)`.
...
mu(T, Y, p0)
D_k(T, Y, p0)
rho_thermo(T, Y, p0) from Cantera; diagnostic in constant-density mode and
active density source in guarded variable-density mode
```

### Density Mode Restriction
...
guarded variable-density mode:
  transport%rho <- energy%rho_thermo
  density_eos = "cantera"
```

`density_eos="cantera"` means the selected Cantera phase supplies density; the
actual EOS is the YAML phase model.
...
### Decision 3: Keep constant-density validation as the baseline

Constant-density validation remains the stable baseline. The guarded
variable-density path should be validated with its own projection, continuity,
species, and energy metrics.
...
### Decision 5: Distinguish diagnostic and active Cantera density

In constant-density mode, `energy%rho_thermo` is diagnostic. In guarded
variable-density mode, `energy%rho_thermo` is synced into active
`transport%rho` after thermo updates.
