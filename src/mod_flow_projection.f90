...
module mod_flow_projection
   ...
   type, public :: solver_stats_t
      ...
      real(rk) :: max_divergence = zero    !< Maximum raw \( \nabla \cdot u \) [1/s]; not the primary variable-density projection metric.
      real(rk) :: rms_divergence = zero    !< RMS raw \( \nabla \cdot u \); use `divu_minus_S_projection` in variable-density mode.
...
contains
   ...
   !!
   !! For pure-Neumann pressure systems the Laplacian is singular with a
   !! constant null vector.  In experimental variable-density mode we keep every
   !! cell's pressure equation active and use this zero-mean gauge instead of
   !! replacing cell 1 with an artificial pinning row.
   subroutine subtract_owned_mean_scalar(flow, scalar)
      ...
      if (ierr /= MPI_SUCCESS) call fatal_error('flow', 'MPI failure reducing zero-mean scalar sum')

      call MPI_Allreduce(local_count, global_count, 1, MPI_INTEGER, MPI_SUM, flow%comm, ierr)
      if (ierr /= MPI_SUCCESS) call fatal_error('flow', 'MPI failure reducing zero-mean scalar count')
   ...
   subroutine solve_pressure_correction(mesh, flow, bc, params, transport, rhs, phi, stats)
      ...
      associate( &
         ...
         if (.not. pressure_cache%has_dirichlet_pressure .and. params%enable_variable_density) then
         ...
         if (.not. pressure_cache%has_dirichlet_pressure) then
            if (params%enable_variable_density) then
               call subtract_owned_mean_scalar(flow, pvec)
            else
               pvec(1) = zero
            end if
         end if
         ...
         do iter = 1, params%pressure_max_iter
            ...
            call pressure_matvec(mesh, flow, bc, params, transport, pvec, local_ap)
   ...
   !!
   !! Returns \(1/\rho_f\), using an arithmetic interpolation of inverse
   !! density for interior/periodic faces and the owner value on physical
   !! boundary faces.  This helper is used by the guarded variable-density
   !! projection branch.
   function pressure_face_inverse_density(transport, owner, nb) result(beta)
      ...
      if (.not. allocated(transport%rho)) then
         call fatal_error('flow', 'transport rho must be allocated for variable-density projection')
      end if
   ...
   subroutine pressure_matvec(mesh, flow, bc, params, transport, x, local_ax)
      use mod_profiler, only : profiler_start, profiler_stop
      type(mesh_t), intent(in) :: mesh
      type(flow_mpi_t), intent(in) :: flow
      type(bc_set_t), intent(in) :: bc
         ...
         do c = flow%first_cell, flow%last_cell
            ...
            do lf = 1, mesh%ncell_faces(c)
               ...
               if (nb > 0) then
                  ...
                  local_ax(c) = local_ax(c) + coeff * (x(c) - x(nb))
               else
                  if (boundary_pressure_type(mesh, bc, f) == bc_dirichlet) then
                     dist = face_normal_distance(mesh, bc, f, c, 0)
                     beta_face = pressure_face_inverse_density(transport, c, 0)
                     coeff = beta_face * mesh%faces(f)%area / dist
                     local_ax(c) = local_ax(c) + coeff * x(c)
                  end if
               end if
            end do
         end do
   ...
   subroutine balance_neumann_outlet_flux(mesh, flow, bc, params, fields, face_flux)
      ...
      call MPI_Allreduce(local_target_flux, global_target_flux, 1, &
                         MPI_DOUBLE_PRECISION, MPI_SUM, flow%comm, ierr)
      call check_mpi(ierr, 'outlet balance target flux')
   ...
   subroutine update_lowmach_divergence_source_from_density(mesh, flow, params, transport, fields)
      type(mesh_t), intent(in) :: mesh
      type(flow_mpi_t), intent(inout) :: flow
      type(case_params_t), intent(in) :: params
      type(transport_properties_t), intent(inout) :: transport
      ...
      real(rk) :: vol, rho_c, rho_old_c, rho_nb, rho_f
      real(rk) :: flux_out, history_source, advective_density_term, advective_source
      real(rk) :: rho_floor
         ...
         if (have_face_flux .and. vol > rho_floor) then
            do lf = 1, mesh%ncell_faces(c)
               ...
               if (nb > 0 .and. nb <= mesh%ncells) then
                  ...
                  ! No explicit density boundary state is available in the
                  ! current staged variable-density path.  Use zero normal
                  ! density gradient on physical boundaries.
                  rho_f = rho_c
               end if
