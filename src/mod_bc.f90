...
contains
   ...
   !! - **Wall/Dirichlet**: Returns the specified patch velocity and therefore
   !!   prescribes volumetric face flux. Mass flux is derived later from the
   !!   active density; no mass-flow boundary condition is implemented here.
   ...
   !!
   !! Dirichlet species boundaries use `patch_Y(k,p)`. All other species
   !! boundary types currently extrapolate the interior mass fraction, so fixed
   !! temperature boundaries without fixed species use the interior composition
   !! when constructing \(h_b=h(T_b,Y_b,p_0)\).
   ...
   !!
   !! This pressure is the hydrodynamic/projection field used by the Poisson
   !! solve. It is not the thermodynamic pressure \(p_0\) passed to Cantera.
